...
class NaiveBayesClassifier():
    ...
    def __classConditionalDensity(self, X, y):
        ...
        for i in self.classes:
            ...
            #sigma_i's
            sum_sigma = np.matrix(np.zeros((np.shape(X[0])[0],np.shape(X[0])[0])))
            for k in range(len(y)):
                if y[k] == i:
    ...
    def predict(self, X_test):     
      ...
      for x in X_test:
          ...
          for i in self.classes:
              ...
              e = -0.5 * (np.matrix(x) - self.u[I]) * sigma_inv * (np.matrix(x) - self.u[I]).T
              exp = np.exp(e[0,0])
...
predictions, probabilities = clf.predict(X_test) # make predictions on new cases
